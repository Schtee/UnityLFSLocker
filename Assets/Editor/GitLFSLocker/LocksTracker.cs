...
namespace GitLFSLocker
{
	class LocksTracker
	{
		...
		public NPath GetRepositoryRelativePath(NPath absolutePath)
		{
			...
			{
				throw new Exception("Tried to get relative path of file outside repository " + absolutePath + " Repo path: " + _repositoryPath);
			}
		...
		public void UnlockAbsolutePath(NPath path, bool useForce)
		{
			Unlock(GetRepositoryRelativePath(path), useForce);
		}

		public void Unlock(NPath path, bool useForce)
		{
			lock (_locks)
			{
			...
			RunCommand(command, (success, message) => HandleUnlocked(success, message, path));
		}
